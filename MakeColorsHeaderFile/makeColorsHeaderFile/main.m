...
int main(int argc, const char * argv[]) {
   @autoreleasepool {
      if (argc == 3)// если передаем аргументы, то argc будет больше 1(в зависимости от кол-ва аргументов)
      {
         ...
         NSMutableString *string = [NSMutableString stringWithFormat:@"@interface UIColor (RMStylishColors)\n\n"];
         for (NSDictionary *dictionary in  colorArray) {
            [string appendFormat:@"+ (UIColor*)%@;\n",dictionary[@"name"]];
         }
         [string appendFormat:@"\n@end"];
         NSError *error;
         [string writeToFile:destLocation atomically:YES encoding:NSUTF8StringEncoding error:&error];
         if (error) {
      ...
      {
         NSLog(@"Invalid arguments count use makeColorsHeaderFile source_plist dest_h");
         return -1;
      }
   }
    return 0;
